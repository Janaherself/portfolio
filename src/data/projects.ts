...
/**
 * Exactly three projects, in display order.
 *
 * Leave `githubUrl` / `liveUrl` empty to hide that button — never invent a
 * URL. Update `status` / `statusNote` as each project evolves.
 */
...
export const projects: Project[] = [
  ...
  },
  {
    slug: 'sorpresa',
    name: 'Sorpresa',
    tagline: 'Shopping, but you find out what you got.',
    motif: 'orders',
    description:
    ...
    contribution:
      'I built the REST API and database layer — schema design, raw SQL queries, and the order/category logic — and the Angular frontend that consumes it. Most of the interesting problems were on the backend: modelling categories and mystery packages in a way that stays sane as rules change.',
    stack: ['Node.js', 'TypeScript', 'Express', 'SQL', 'Angular'],
    status: 'in-progress',
